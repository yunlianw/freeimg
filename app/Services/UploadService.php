...
class UploadService
{
    ...
    public function upload(array $file, int $userId, array $opts = []): array
    {
        ...
        if ($isOriginal) {
            ...
            $tempDst = $this->makeTempFile('', $ext) . '.' . $ext;

            $result = $this->processor->compress($tempSrc, $tempDst, [
                'max_width'  => $maxW,
                'max_height' => $maxH,
                'quality'    => $quality,
                'format'     => $ext,
            ...
            if (!$result['success']) {
                @unlink($tempDst);
                return $this->fail('压缩失败: ' . ($result['error'] ?? '未知错误'));
            }

            $storedContent = file_get_contents($tempDst);
            @unlink($tempDst);
